								...
								<div class="card-body">
									...
									<div class="card card-custom gutter-b example example-compact">
										<div class="card-header">
											<h3 class="card-title"> {{  __('langs.edit_product') }}</h3>
											<div class="card-toolbar">
										...
										<!--begin::Form-->
										 {!! Form::open(['url' => route('admin.product.edit',['id' => $product->id]),
														'method'=>'post' ,
												...
												<div class="col-lg-6">
													<label>
														  ...
														  @if(app()->isLocale('ar'))
															...
															@endif
													: </label>
														<input id="title" type="text" class="form-control" required  placeholder=" {{  __('langs.name') }}" name="name"   value="{{ $product->name }}" />
												...
												<div class="col-lg-6">
													...
													{{  __('langs.name') }}
														   ...
														   @if(app()->isLocale('ar'))
															...
															@endif
													 : </label>
														<input id="title" type="text" class="form-control" required  placeholder=" {{  __('langs.name') }}" name="name_en"   value="{{ $product->name_en }}" />
											...
											<div class="form-group row">
												...
												<div class="col-lg-6">
													...
													<input id="title" type="text" class="form-control" required  placeholder=" {{  __('langs.sku') }}" name="sku"   value="{{ $product->sku }}" />
											...
											<div class="form-group row">
												<div class="col-lg-6">
													...
													<input id="title" type="text" class="form-control" required  placeholder=" {{  __('langs.quantity') }}" name="quantity"   value="{{ $product->quantity }}" />
												...
												<div class="col-lg-6">
													...
													<input id="title" type="text" class="form-control" required  placeholder=" {{  __('langs.min_stock') }}" name="min_stock"   value="{{ $product->min_stock }}" />
												...
												<div class="col-lg-12">
													...
													<select class="form-control" name= "categories[]" multiple="multiple">
														@foreach($categories as $category)
															@if(app()->isLocale('ar'))
															<option value="{{  $category->id }}" {{ in_array($category->id,$categories_for_product_id_arr) ? 'selected' :  ''}}>{{  $category->name }}</option>
															@endif

															@if(app()->isLocale('en'))
															<option value="{{  $category->id }}" {{ in_array($category->id,$categories_for_product_id_arr) ? 'selected' :  ''}}>{{  $category->name_en }}</option>
															@endif
														@endforeach
											...
											<div class="form-group row">
													<div class="col-lg-12">
														<label>
													    	{{  __('langs.similar_product') }} :
													    </label>
														<select class="form-control" name= "similars[]" multiple="multiple">
															@foreach($products as $pro)
																@if(app()->isLocale('ar'))
																<option value="{{  $pro->id }}" {{ in_array($pro->id,$similars_for_product_id_arr) ? 'selected' :  ''}}>{{  $pro->name }}</option>
																@endif

																@if(app()->isLocale('en'))
																<option value="{{  $pro->id }}" {{ in_array($pro->id,$similars_for_product_id_arr) ? 'selected' :  ''}}>{{  $pro->name_en }}</option>
																@endif
															@endforeach
											...
											<div class="form-group row">
												<div class="col-lg-12">
													...
													: </label>
														...
														<textarea class="form-control" name="description_en"   rows="4">{{$product->description_en}}</textarea>
	...
	<script type="text/javascript">
		$(document).ready(function() {
		  ...
		  $("body").on("click",".removeImage",function(){
				 ...
				 console.log("lassst : ", $(".del_img").val());
				// var id = $(".id_img").val();
				 ...
				 if($(".del_img").val()){
				  ...
				  console.log("ids befor : ",ids);
				  ids += (ids == "") ?  id : (","+id);
				  console.log("ids after : ",ids);
				  $(".del_img").val(ids);
				  console.log("lassst : ", $(".del_img").val());

				  $(this).parent(".rem_img").remove();
			});

			//Color
			$("body").on("click",".removeColor",function(){

				 console.log("lassst : ", $(".del_color").val());
				// var id = $(".id_color").val();
				 var id = $(this).parent().attr('id');
				 console.log("id : ",id);

				 if($(".del_color").val()){
					 var ids = $(".del_color").val();
				 }else{
					 var ids = "";
				 }
